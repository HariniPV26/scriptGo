...
import {
    ...
    MonitorPlay, Linkedin, Instagram, Calendar, Image as ImageIcon,
    FileText, Layout, Play, Fullscreen
} from 'lucide-react'
...
export default function Editor({ initialData, scriptId }: EditorProps) {
    ...
    const handleSave = async () => {
        ...
        const scriptData = {
            ...
            platform,
            content: { text: content, visuals: visuals }
        }
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen overflow-hidden bg-background font-sans selection:bg-primary/30">
            ...
            <div className={`${showMobileSidebar ? 'fixed inset-0 z-50 bg-background/95 backdrop-blur-xl p-6' : 'hidden md:flex w-[340px] bg-background border-r border-white/5 p-8'
                ...
                <form onSubmit={handleGenerate} className="space-y-8 flex-1 overflow-y-auto pb-6 scrollbar-none">
                    <div className="space-y-4">
                        <label className="text-[11px] font-black text-muted-foreground uppercase tracking-[0.2em] flex items-center justify-between">Platform</label>
                        <div className="grid grid-cols-2 gap-3">
                            {[
                                ...
                                <button
                                    ...
                                    className={`p-4 rounded-2xl text-[10px] font-black uppercase transition-all duration-300 flex flex-col items-center gap-3 border-2 ${platform === p.name ? 'bg-primary/10 border-primary text-primary shadow-xl shadow-primary/10' : 'bg-white/5 border-transparent text-muted-foreground hover:bg-white/10'}`}
                                >
                    ...
                    <div className="space-y-4">
                        ...
                        <textarea
                            ...
                            className="w-full px-5 py-4 bg-white/5 border border-white/5 rounded-2xl text-foreground focus:ring-2 focus:ring-primary/40 h-32 resize-none placeholder:text-muted-foreground/30 font-medium text-sm transition-all"
                        />
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-3">
                                ...
                                <select value={language} onChange={(e) => setLanguage(e.target.value)} className="w-full h-11 px-4 bg-white/5 border border-white/5 rounded-xl text-foreground cursor-pointer text-[10px] font-bold">
                            ...
                            <div className="space-y-3">
                                ...
                                <select value={framework} onChange={(e) => setFramework(e.target.value)} className="w-full h-11 px-4 bg-white/5 border border-white/5 rounded-xl text-foreground cursor-pointer text-[10px] font-bold">
                    ...
                    <div className="flex flex-col gap-3 mt-4">
                        ...
                        {!isCalendarMode && content && (
                            <button
                                ...
                                disabled={loading || generatingVisuals}
                                className={`w-full py-4 relative overflow-hidden bg-white/5 border border-white/10 text-foreground font-bold uppercase tracking-[0.1em] rounded-2xl transition-all active:scale-[0.98] ${generatingVisuals ? 'animate-pulse' : 'hover:bg-amber-500/10 hover:border-amber-500/30'}`}
                            >
                                <div className="flex items-center justify-center gap-2 relative z-10">
                                    {generatingVisuals ? <Loader2 className="h-4 w-4 animate-spin text-amber-500" /> : <ImageIcon className="h-4 w-4 text-amber-500" />}
                                    <span className="text-[10px]">Visual Storyboard</span>
                                </div>
                            </button>
                        )}
                    </div>
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden relative bg-[#FAF9F6] dark:bg-[#0D0D0E]">
                {/* Status Bar */}
                <div className="h-24 border-b border-black/[0.03] dark:border-white/5 bg-background/50 backdrop-blur-3xl flex items-center justify-between px-10 z-10 shrink-0">
                    <div className="flex items-center gap-6 w-full max-w-xl">
                        <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                <span className={`h-1.5 w-1.5 rounded-full ${saving ? 'bg-primary animate-pulse' : 'bg-emerald-500'}`}></span>
                                <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground opacity-60">
                                    {saving ? 'Syncing...' : 'Live Workspace'}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        {/* View Mode Toggles */}
                        {visuals && (
                            <div className="bg-black/5 dark:bg-white/5 p-1 rounded-xl flex items-center gap-1 mr-4">
                                <button
                                    onClick={() => setViewMode('script')}
                                    className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${viewMode === 'script' ? 'bg-primary text-white shadow-lg' : 'text-muted-foreground hover:text-foreground'}`}
                                >
                                    <FileText className="h-3 w-3" /> Script
                                </button>
                                <button
                                    onClick={() => setViewMode('storyboard')}
                                    className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${viewMode === 'storyboard' ? 'bg-primary text-white shadow-lg' : 'text-muted-foreground hover:text-foreground'}`}
                                >
                        ...
                        <button onClick={handleCopy} className="h-10 px-4 flex items-center gap-2 text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-all rounded-xl text-xs font-bold">
                            <Copy className="h-4 w-4" />
                        </button>
                        <button onClick={handleEmail} disabled={sendingEmail} className="h-10 px-4 flex items-center gap-2 text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-all rounded-xl text-xs font-bold">
                        ...
                        <button onClick={handleSave} disabled={saving || !content} className="h-11 px-6 bg-foreground text-background dark:bg-white dark:text-black rounded-xl font-black text-xs uppercase tracking-widest transition-all hover:scale-[1.05] shadow-xl active:scale-95">
                ...
                {/* Content Area */}
                <div className="flex-1 overflow-auto p-12 relative z-0 custom-scrollbar flex flex-col items-center bg-dots-grid">
                    {!content ? (
                        <div className="h-full w-full flex flex-col items-center justify-center text-muted-foreground space-y-8 max-w-2xl text-center">
                            ...
                            <div className="space-y-3">
                                ...
                                <p className="text-muted-foreground/80 font-medium leading-relaxed max-w-sm mx-auto">
                                    Bring your vision to life with zero friction.
                                </p>
                            </div>
                        </div>
                    ) : (
                        <div className="w-full max-w-5xl h-full pb-20 animate-in fade-in slide-in-from-bottom-8 duration-700">
                            {viewMode === 'script' ? (
                                <div className="saas-card p-0 h-full border-black/[0.03] dark:border-white/5 bg-background dark:bg-background/40 shadow-premium overflow-hidden">
                                    <textarea
                                        value={content}
                                        ...
                                        placeholder="Generating masterpiece..."
                                        spellCheck={false}
                                    />
                                </div>
                            ) : (
                                <div className="space-y-12 animate-in fade-in slide-in-from-right-8 duration-700">
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                        {visuals?.map((shot: any, idx: number) => (
                                            <div key={idx} className="group relative bg-background/40 backdrop-blur-3xl border border-white/5 rounded-[2rem] overflow-hidden hover:border-primary/30 transition-all hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-2 saas-card p-0">
                                                <div className="aspect-video relative overflow-hidden bg-white/5">
                                                    <img
                                                        src={`https://image.pollinations.ai/prompt/${encodeURIComponent(shot.visualPrompt + " high quality cinematic movie still, masterwork, 8k, detailed, " + platform + " style")}`}
                                                        alt={shot.shot}
                                                    ...
                                                    <div className="absolute bottom-4 left-6 flex items-center gap-2">
                                                        ...
                                                        <span className="text-[10px] font-black uppercase text-white tracking-widest">{shot.shot}</span>
                                                    </div>
                                                </div>
                                                <div className="p-8 space-y-4">
                                                    <p className="text-sm font-medium leading-relaxed text-foreground/80 opacity-90 group-hover:opacity-100 transition-opacity">
                                                        {shot.description}
                                                    </p>
                                                    <div className="flex items-center justify-between pt-4 border-t border-white/5">
                                                        <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground opacity-40">AI Storyboard Gen</span>
                                                        <button className="h-8 w-8 rounded-full bg-white/5 flex items-center justify-center hover:bg-primary/20 hover:text-primary transition-all">
                                    ...
                                    <div className="flex justify-center pb-20">
                                        ...
                                        >
                                            <FileText className="h-4 w-4" /> Back to script editor
                                        </button>
                                    </div>
                                </div>
                            )}
